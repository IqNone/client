...
import static el.protocol.ProtocolTypes.ADD_ACTOR_COMMAND;
import static el.protocol.ProtocolTypes.ADD_NEW_ACTOR;
import static el.protocol.ProtocolTypes.ADD_NEW_ENHANCED_ACTOR;
import static el.protocol.ProtocolTypes.CHANGE_MAP;
import static el.protocol.ProtocolTypes.GET_ACTIVE_CHANNELS;
import static el.protocol.ProtocolTypes.GET_ITEMS_COOLDOWN;
import static el.protocol.ProtocolTypes.GET_NEW_INVENTORY_ITEM;
import static el.protocol.ProtocolTypes.GET_TRADE_ACCEPT;
import static el.protocol.ProtocolTypes.GET_TRADE_EXIT;
import static el.protocol.ProtocolTypes.GET_TRADE_OBJECT;
import static el.protocol.ProtocolTypes.GET_TRADE_PARTNER_NAME;
import static el.protocol.ProtocolTypes.GET_TRADE_REJECT;
import static el.protocol.ProtocolTypes.GET_YOUR_TRADEOBJECTS;
import static el.protocol.ProtocolTypes.HERE_YOUR_INVENTORY;
import static el.protocol.ProtocolTypes.HERE_YOUR_STATS;
import static el.protocol.ProtocolTypes.INVENTORY_ITEM_TEXT;
import static el.protocol.ProtocolTypes.KILL_ALL_ACTORS;
import static el.protocol.ProtocolTypes.LOG_IN_NOT_OK;
import static el.protocol.ProtocolTypes.LOG_IN_OK;
import static el.protocol.ProtocolTypes.NEW_MINUTE;
import static el.protocol.ProtocolTypes.NPC_OPTIONS_LIST;
import static el.protocol.ProtocolTypes.PING_REQUEST;
import static el.protocol.ProtocolTypes.RAW_TEXT;
import static el.protocol.ProtocolTypes.REMOVE_ACTOR;
import static el.protocol.ProtocolTypes.REMOVE_ITEM_FROM_INVENTORY;
import static el.protocol.ProtocolTypes.REMOVE_TRADE_OBJECT;
import static el.protocol.ProtocolTypes.SEND_PARTIAL_STAT;
import static el.protocol.ProtocolTypes.STORAGE_ITEMS;
import static el.protocol.ProtocolTypes.STORAGE_LIST;
import static el.protocol.ProtocolTypes.YOU_ARE;
import static el.protocol.ProtocolTypes.YOU_DONT_EXIST;
...
public class Client implements MessageReceiveListener {
    ...
    // };
    // El 1.9.3
    public static final byte[] VERSION = {0x0a, 0x0f, 0x00, 0x0a, 0x00, 0x1b,
    ...
    public void onMessageReceived(Message message) {
        synchronized (actor) {
            try {
                LOGGER.info("Processing message of type " + message.getType() + " and source " + ByteUtils.byteArrayToHexString(message.getSource()));
                tryProcessMessage(message);
            } catch (Exception e) {
                LOGGER.error(
                        "error while processing " + ByteUtils.byteArrayToHexString(message.getSource()),
                        e);
            }
        }
    }
    ...
    private void onGetActiveChannels(Message message) {
        LOGGER.info("Getting Active Channels");
        byte[] bytes = message.getSource();
        actor.activeChannel = ByteUtils.unsigned(bytes[3]);
        actor.channels.set(0, ByteUtils.nex4(bytes, 4));
        actor.channels.set(1, ByteUtils.nex4(bytes, 8));
        actor.channels.set(2, ByteUtils.nex4(bytes, 12));
    }
    ...
    private void onRawText(Message message) {
        ...
        // From desktop client: multiplayer.c
//        if (data_length <= 4)
//        {
...
//        }
        ...
        //TODO handle server popup channel.
        ...
        for (Span span : actor.texts.get(actor.texts.size() - 1).spans) {
            if (span.text.contains("You started to harvest")) {
                ...
                return;
            } else if (actor.texts.get(actor.texts.size() - 1).channel == 1
                    && span.text.startsWith("[PM from ")) {
                String last_PM_from = span.text;
                String[] split = last_PM_from.split(":");
                last_PM_from = split[0].substring(9);

                actor.last_PM_from = last_PM_from;
            }
        }

        LOGGER.info(new String(message.getSource(), 4,
                message.getSource().length - 5));
    }
    ...
    private void onChangeMap(Message message) {
        String map = new String(message.getSource(), 4, message.getLength() - 3)
                + "a";// elm->elma
    ...
    private void onNpcOptionsList(Message message) {
        ...
        if (options.size() > 0
                && options.get(0).text.equals(OPEN_STORAGE_OPTION)) {
            connection.sendMessage(Messages.respondToNpc(
                    options.get(0).toActorId, options.get(0).id));
        }
    }

    private List<NpcOption> getOptionsList(Message message) {
        List<NpcOption> options = new ArrayList<NpcOption>();
        ...
        while (startOption < message.getSource().length) {
            ...
            option.text = new String(message.getSource(), startOption + 2,
                    textLength - 1);
            option.id = ByteUtils.nex2(message.getSource(), startOption + 2
                    + textLength);
            option.toActorId = ByteUtils.nex2(message.getSource(), startOption
                    + 2 + textLength + 2);
    ...
    private void onGetTradePartnerName(Message message) {
        ...
        actor.trade.partnersName = new String(message.getSource(), 4,
                message.getLength() - 2);
    }

    private void onGetTradeObject(Message message) {
        int pos = message.getSource()[10];
        Item items[] = message.getSource()[11] == 0 ? actor.trade.myItems
                : actor.trade.hisItems;
        if (items[pos] == null) {
            items[pos] = new Item();
        }
    ...
    private void onRemoveTradeObject(Message message) {
        ...
        Item items[] = message.getSource()[8] == 0 ? actor.trade.myItems
                : actor.trade.hisItems;
    ...
    public void enter(int entrableId) {
        ...
        // spans.add(new Span(Colors.COLORS[Colors.BLUE1], "object id " +
        // entrableId));
    ...
    public void putObjectOnTrade(int itemPos, int quantity, boolean fromStorage) {
        connection.sendMessage(Messages.putObjectOnTrade(itemPos, quantity,
                fromStorage));
    }

    public void removeObjectFromTrade(int itemPos, int quantity) {
        connection.sendMessage(Messages
                .removeObjectFromTrade(itemPos, quantity));
    }
    ...
    public void acceptTrade() {
        connection.sendMessage(Messages.acceptTrade(actor.trade.myAccept,
                actor.trade.hisItems));
    }
    ...
    public void manufacture_item(Item[] manufacture_items, int mix_num) {
        connection
                .sendMessage(Messages.manufacture(manufacture_items, mix_num));
    }
